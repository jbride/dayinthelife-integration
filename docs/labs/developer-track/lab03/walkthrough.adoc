...
. Click on the route link and this would open in a new tab. 

.. Append `api-doc` to the URI.  This will display the OpenAPI documentation of the RESTful service

.. Replace the `api-doc` URI context with `/locations`.  As a result, you should receive a list of all locations.
+
